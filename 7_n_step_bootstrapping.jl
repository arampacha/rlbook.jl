...
function random_step(state::Int64, n::Int64=19)
...
# ╔═╡ 4d68f26b-1740-456e-a032-4343816c0353
mod(10,10) + 1 

# ╔═╡ 78303981-d6c3-4ae6-b395-edf46fcbaa8d
function mrp_td(n=1, α=0.1, init=0.5, γ=1., n_states=19)
...
# ╔═╡ 66ef9027-c52a-4498-95aa-4b5ee2b15bec
mrp_td(5)

# ╔═╡ 31afbb6e-3948-474c-8493-e5cf456554eb
function test()
	n=2
	α=0.1
	init=0.5
	...
	n_states=3
	V = zeros(n) .+ init
	# vs = Array{Float64}(undef, (11, n))
	# vs[1, :] = V
	# max_steps = 1_000_000
	modn(x) = mod(x, n) + 1
	# for i = 1: 10
	rewards = Array{Int64}(undef,n)
	states = Array{Int64}(undef,n)
	s = n_states ÷ 2 + 1
	T = Inf
	t = 0
	for i = 1:2
		...
		if τ ≥ 0
			# G = sum(
			return [γ^(i-τ-1)*rewards[modn(i)] for i in (τ+1):Int(min(τ+n, T))]
			# )
			if τ + n < T
				G += γ^n*V[states[modn(τ+n)]]
			end
			upd_state = states[modn(τ)]
			if !(upd_state in (0, n_states+1))
				V[upd_state] += α*(G - V[upd_state])
...
# ╔═╡ c2af7be1-5865-4003-8ba2-2260747141d5
mod(-10, 5) + 1

# ╔═╡ 06a36677-efc4-457f-a03c-690767e10577
typeof(Int(min(1, 2.)))

# ╔═╡ cfcebd86-c522-4a5c-b4a2-d6cfb12eea82
let
	a = [1,2,3]
	getindex
end
